...
WordPress: `devree-kadence-child/inc/faq-discovery.php`, `archive-faq.php`, `assets/js/dv-faq-search.js`. Alle gepubliceerde vragen staan in HTML; zoeken/filteren is progressieve JavaScript. Zoekacties worden bij Zoeken/Enter, een onderwerpwijziging met zoekterm of het openen van een zoekresultaat vastgelegd met Matomo `trackSiteSearch(query, 'FAQ / Onderwerp', resultCount)`. Nulresultaten krijgen daarnaast een event `FAQ zonder resultaat`, omdat het standaard Matomo-nulresultatenrapport termen weglaat die ook positieve resultaten hadden. Het platform telt deze events per zoekterm. Geen ruwe invoer in URL's. Mogelijke e-mailadressen, telefoonnummers en URLs worden niet gemeten. Analytics blijft afhankelijk van trackingtoestemming, browserblokkades en de bestaande Matomo-configuratie; dit is geen volledige registratie van alle bezoekers.
