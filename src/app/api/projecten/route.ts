...
export async function PATCH(request: NextRequest) {
  ...
  // Aankooptarieven — null betekent platformdefault
  ...
  if (updateData.type !== undefined) {
    const current = await prisma.project.findUnique({ where: { id: data.id }, select: { type: true } });
    if (current && current.type !== updateData.type) {
      await prisma.projectProposal.updateMany({
        where: { projectId: data.id, status: "OPEN" },
