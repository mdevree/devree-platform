...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [appointments, confirmations, drafts, calls, mautic, pbx] = await Promise.all([
    ...
    prisma.pbxRequest.findMany({ where: { contactId }, include: { task: { select: { status: true } } } }),
  ]);
  ...
  for (const r of pbx) items.push({ id: `pbx:${r.id}`, at: r.receivedAt.toISOString(), category: "calls", title: `${r.kind === "callback" ? "Terugbelverzoek" : r.kind === "viewing" ? "Woninglink gevraagd" : "Gemiste oproep"}${r.task ? ` · ${r.task.status}` : ""}`, detail: r.notes || undefined, source: "PBX-opvang" });
